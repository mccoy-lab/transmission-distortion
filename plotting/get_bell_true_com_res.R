...
assess_it <- function(dir_base1, sample, sample_desc, dir_base2, chr){
  ...
  imputation_assess <- rhapsodi::sim_assess_gam_imputation(fake_true_gametes, rhapsodi_gametes, num_snps, num_gametes)
  imputation_completeness <- imputation_assess$com
  ...
  out <- data.frame(sample = sample, sample_desc = sample_desc,  chr = chr, phase_com = phasing_completeness, imp_com = imputation_completeness, res = res_vec_str)
  return(out)
}
