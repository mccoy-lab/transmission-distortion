...
plot_fn_fp <- function(cov, rsds, storage_list){
  tall_df <- data.frame()
  for (rsd in rsds){
    tall_df <- rbind(tall_df, melt(merge(storage_list[[paste0("c",cov)]][[paste0("df",rsd)]]$fn, storage_list[[paste0("c",cov)]][[paste0("df",rsd)]]$fp, all = TRUE, by = "gam")))  
  }
  p <- ggplot(tall_df, aes(as.factor(gam), value, shape=factor(variable))) + theme_bw() + theme(panel.grid = element_blank(), axis.text.x=element_blank())
  p + geom_point(aes(colour = factor(variable), alpha=1/10)) + labs(y='relative location', x="gamete") + ggtitle(paste0("Coverage: ", cov))
  ggsave(paste0("fn_fp_by_gamete_cov",cov,".png"))
}

for (cov in covs){
  plot_fn_fp(cov, rsds, storage_list)
}
