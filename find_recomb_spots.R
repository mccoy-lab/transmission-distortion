...
test7 <- c("h1", "h1", "h1", "h1", "h1", "h1", "h1", "h1", NA, NA, "h2", "h1", NA, "h2", "h2" ) #this will catch all 3 recombination spots now
overall_test <- cbind(test1, test2, test3, test4, test5, test6, test7)
...
hap_locs <- which(!is.na(rle_result$values))
neighboring_haps <- diff(hap_locs)
lens_fresult <- lapply(rle_result["lengths"], cumsum)$lengths
if (length(neighboring_haps) > 0) { #this section is in case there is atleast one recombination spot without NAs buffering it, but there are NAs buffering others
  for (i in 1:length(neighboring_haps)) {
    if (neighboring_haps[i] == 1){
      rspot_end_loc <- lens_fresult[hap_locs[i+1]]
      recomb_spots <- rbind(recomb_spots, c(ident, rspot_end_loc-1, rspot_end_loc))
    }
  }
}
...
recomb_spots_all <- do.call(rbind, pblapply(1:ncol(overall_test), function(x) find_recomb_spots(overall_test, x, c('test1', 'test2', 'test3', 'test4', 'test5', 'test6', 'test7'))))