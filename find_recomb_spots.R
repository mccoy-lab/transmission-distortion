...
#test <- c("h1", "h1", "h1", "h1", "h1", "h1", "h1", "h1", NA, NA, "h2", "h1", NA, "h2", "h2" ) #this will catch two of the 3 recombination spots
...
message(paste("neighboring_difs: ", neighboring_difs))

subsetted_na_locs <- c(na_locs[1])
message(paste("subsetted_na_locs: ", subsetted_na_locs))
...
if ((last_na_loc - first_na_loc + 1) == num_na_locs) { #there's a single recombination spot
  ...
  message(paste('Added spot ', ident, first_na_loc -1, last_na_loc + 1))
  ...
  while ((last_na_loc - first_na_loc + 1) > num_na_locs) { #means that we must have some haplotypes in the middle or there must be more than one recombination spot
    ...
    message(paste("first_hp_loc: " , first_hp_loc))
    message(paste("i: ", i))
    message(paste("first_na_loc -1: ", first_na_loc-1))
    message(paste("first_na_loc_fi + first_hp_loc -1: ", first_na_loc_fi+first_hp_loc -1))
    message(paste("first_na_loc_fi: ", first_na_loc_fi))
    message(paste("num_na_locs: ", num_na_locs))
    if (!is.na(first_hp_loc)) { #for all but the last recombination spots
      ...
      message(paste('Added spot ', ident, first_na_loc -1, first_na_loc_fi+first_hp_loc-1))
      ...
      message(paste("new first_na_loc: ", first_na_loc))
      ...
      message(paste("new num_na_locs: ", num_na_locs))
      if (num_na_locs == 0){ #this also means reached last recombination spot
        break
      }
