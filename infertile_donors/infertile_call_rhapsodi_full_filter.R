...
#separate positions from genotypes
# and remove any non-heterozygous SNPs
hetRows <- ((rowSums(full_data[,-1] == 0, na.rm = TRUE) > 0) & (rowSums(full_data[, -1] == 1, na.rm=TRUE) > 0))
positions <- full_data[hetRows,1]
dt <- full_data[hetRows, -1]

input_out <- list(dt=dt, positions=positions)
...
positions_dt <- input_out$positions %>% as.data.table() %>% `colnames<-`("pos") #had to name the column
positions_dt[, chr := col_chr_name]
positions_dt[, start := pos]
positions_dt[, end := pos]
...
dt_filtered_3 <- dt_filtered_2[dt_filtered_2$start %in% raw_data_in_giab_union$positions]
...
dt_filtered_4 <- dt_filtered_3[dt_filtered_3$start %in% raw_data_in_1kgp_snps$positions]

# Use the newly filtered set of SNPs to subset the raw data
dt <- input_out$dt[input_out$positions %in% dt_filtered_4$start,]
positions <- input_out$positions[input_out$positions %in% dt_filtered_4$start]

#5. Remove SNPs that have more reads than expected
allele_counts <- data.table(x = rowSums(dt == 1, na.rm = TRUE),
                            y = rowSums(dt == 0, na.rm = TRUE))
allele_counts$positions <- positions
...
dt_filtered_5 <- dt[positions %!in% allele_counts_high$positions,]
positions_5 <- positions[positions %!in% allele_counts_high$positions]

# Use dt for downstream analyses
dt <- cbind(positions_5, dt_filtered_5)
